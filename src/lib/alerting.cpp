...
namespace monitoring {
  ...
  void alerting::push_log(const traffic_log& log) {
    ...
    store_count(m_hit_count, log.m_date);
    compute();
  }

  bool alerting::complete_period_stored() {
    //Wait a delay before considering the first date
    if (!m_window_start_time && m_most_recent_date - m_hit_count.begin()->first >= m_wait_delay) {
  ...
  void alerting::traffic_alert_status_changed(int hit_count_average, long long window_end_date) {
    m_alert = !m_alert;
    traffic_alert alert;
  ...
  void alerting::compute_traffic_alert() {
    while (complete_period_stored()) {
      ...
      long long startDate = m_window_start_time;
      long long endDate = m_window_start_time + m_window_size - 1;

      if (!m_first_window_computed) {
        //First window: cumulate hits of every date received in the window
        auto lower_bound = m_hit_count.lower_bound(startDate);
        auto upper_bound = m_hit_count.upper_bound(endDate);
