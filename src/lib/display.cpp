...
namespace monitoring {
  namespace display {
    ...
    void most_hit_sections(std::stringstream& stream, const window_metrics& metrics) {
    ...
    void count_per_operation(std::stringstream& stream, const window_metrics& metrics) {
       stream << "Operations: ";
       http_verb verbs[] = { http_verb::Post, http_verb::Get, http_verb::Put, http_verb::Patch, http_verb::Delete };
       for (auto verb : verbs) {
    ...
    void success_rate(std::stringstream& stream, const window_metrics& metrics) {
      stream << "Success rate: " << static_cast<int>(metrics.m_success_rate) << "%" << std::endl
        << "Remote host with lowest success rate: " << metrics.m_lowest_success_rate_host 
        << " (" << static_cast<int>(metrics.m_lowest_success_rate) << "%)" << std::endl;
    }
    ...
    void metrics(const window_metrics& metrics) {
      ...
      stream << "-----" << std::endl
        << "Time window: " 
        << local_display_time(metrics.m_start_date) << ", " 
        << local_display_time(metrics.m_end_date) << ":" 
        << std::endl;
    ...
    void alert(const traffic_alert& alert) {
      ...
      if (alert.m_alert) {
        stream << "*** High traffic generated an alert - hits = " << alert.m_hit_count_average
          << ", triggered at " << end_period << " ***";
      }
      else {
