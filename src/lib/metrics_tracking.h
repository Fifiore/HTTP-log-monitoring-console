...
namespace monitoring {
  ...
  private:
    ...
    //Map + most recent date to emulate a circular buffer
    ...
    void success_rate(const std::unordered_map<std::string_view, status_count>& stats_per_remote_host, window_metrics& metrics);
    void increment_per_status(std::unordered_map<std::string_view, status_count>& stats, std::string_view host, bool success);
  };
