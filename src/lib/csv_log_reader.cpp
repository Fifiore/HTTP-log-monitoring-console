...
namespace monitoring {
  ...
  csv_log_reader::csv_log_reader(const std::string& path, char delimiter) : m_delimiter(delimiter) {
    if ("-" == path) {
      m_stream = &std::cin;
    }
    else {
  ...
  void csv_log_reader::fill_log(traffic_log& log, const std::string& value, int value_index) {
  ...
  bool for_each_value_of_next_line(std::istream* stream, char delimiter, Func function) {
    if (!stream)
      return true;
  ...
  std::optional<traffic_log> csv_log_reader::get_line() {
    ...
    traffic_log result;
    bool get_line_succeeded = for_each_value_of_next_line(m_stream, m_delimiter,
      [&](std::string& value, int index) {
        remove_quotes(value);
