...
namespace monitoring {
  ...
  void metrics_tracking::compute_most_hit_sections(const std::unordered_map<std::string, int>& hit_per_section, window_metrics& metrics) {
    ...
    for (const auto& stat : hit_per_section) {
      ...
      if (highestCount < stat.second) {
        highestCount = stat.second;
        metrics.m_most_hit_sections.clear();
        metrics.m_most_hit_sections.push_back(stat.first);
      }
      else if (highestCount == stat.second) {
        metrics.m_most_hit_sections.push_back(stat.first);
      }
    }
    metrics.m_most_hit_section_count = highestCount;
  }

  void metrics_tracking::increment_per_status(std::unordered_map<std::string_view, status_count>& stats, std::string_view host, bool success) {
  ...
  void metrics_tracking::success_rate(const std::unordered_map<std::string_view, status_count>& stats_per_remote_host, window_metrics& metrics) {
    int successCount = 0;
    int requestCount = 0;
    ...
    for (const auto& hostStat : stats_per_remote_host) {
      ...
      requestCount += host_request_count;
      successCount += hostStat.second.m_success_count;
    }
    if (requestCount) {
      double success_rate = successCount * 100 / static_cast<double>(requestCount);
  ...
  void count_http_operations(window_metrics& metric, const std::string& request) {
  ...
  void metrics_tracking::compute_metrics() {
    while (complete_period_stored()) {
      ...
      auto lower_bound = m_logs.lower_bound(metrics.m_start_date);
      auto upper_bound = m_logs.upper_bound(metrics.m_end_date);

      //For every date received in the window
      std::for_each(lower_bound, upper_bound, [&](const auto& date_entries) {
        for (const auto& log : date_entries.second) {
          store_count(hit_per_section, extract_section(log.m_request));
          //Aggregate hits per HTTP verb (PUT,GET...)
          count_http_operations(metrics, log.m_request);
          if (log.m_status > 0) {
            increment_per_status(stats_per_remote_host, log.m_remote_host, log.m_status < 300);
          }
        }
        });
      //Fill metrics
      compute_most_hit_sections(hit_per_section, metrics);
      success_rate(stats_per_remote_host, metrics);
      //Push in output pipe
