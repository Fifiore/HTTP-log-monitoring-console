...
void compute_most_hit_sections(
    const std::unordered_map<std::string, int> &hit_per_section,
...
void metrics_tracking::increment_status_count(
    std::unordered_map<std::string_view, status_count> &stats,
    std::string_view host, bool success) {
...
void metrics_tracking::compute_success_rate(
    const std::unordered_map<std::string_view, status_count>
        &stats_per_remote_host,
    window_metrics &metrics) {
  int success_count = 0;
  int request_count = 0;
  ...
  for (const auto &hostStat : stats_per_remote_host) {
    int host_request_count =
        hostStat.second.m_success_count + hostStat.second.m_failure_count;
    double host_success_rate = hostStat.second.m_success_count * 100 /
                               static_cast<double>(host_request_count);
  ...
  if (request_count) {
    double success_rate =
        success_count * 100 / static_cast<double>(request_count);
...
bool metrics_tracking::complete_period_stored() {
  ...
  if (!m_window_start_time &&
      m_most_recent_date - m_logs.begin()->first >= m_wait_delay) {
...
void increment_http_operations_count(window_metrics &metric,
  ...
  while (complete_period_stored()) {
    ...
    std::for_each(start_bound, end_bound, [&](const auto &date_entries) {
      for (const auto &log : date_entries.second) {
        ...
        if (log.m_status > 0) {
          increment_status_count(stats_per_remote_host, log.m_remote_host,
                                 log.m_status < 300);
        }
      }
    });
