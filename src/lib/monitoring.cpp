...
namespace monitoring {
  ...
  //Worker to read the input log pipe and send it to metrics tracking and alerting
  void log_worker(metrics_tracking& metrics_tracking, alerting& alerting, channel<traffic_log>& pipe) {
    while (true) {
  ...
  void read_and_compute_logs(csv_log_reader& reader, metrics_tracking& metrics_tracking, alerting& alerting) {

    //Initate communications channels and associate it to metrics and alerts
    //Launch workers

    channel<window_metrics> output_metrics_pipe;
    std::thread output_metrics_thread([&]() { display_metrics_worker(output_metrics_pipe); });
    metrics_tracking.set_output_channel(&output_metrics_pipe);

    channel<traffic_alert> output_alert_pipe;
    std::thread output_alert_thread([&]() { display_alert_worker(output_alert_pipe); });
    alerting.set_output_channel(&output_alert_pipe);

    channel<traffic_log> log_pipe;
    std::thread log_worker_thread([&]() { log_worker(metrics_tracking, alerting, log_pipe); });
